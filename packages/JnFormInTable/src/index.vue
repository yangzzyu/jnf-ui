<template>
  ...
  >
    ...
    <template v-for="column in columns" :key="column">
      <el-table-column
        v-if="['index', 'selection', 'expand'].includes(column.type)"
      ...
      <el-table-column
      v-if="!['index', 'selection', 'expand','handle'].includes(column.type)"
        v-bind="{ ...column.columnConfig, ...$attrs }"
        show-overflow-tooltip
      >
        <!-- :index="column.allSort ? typeIndex : 'index'" -->
        <template #header>
          <i v-if="column.required && !props.disabled" style="color: #f56c6c"
            >*</i
          >
          {{ column.label }}
        </template>
        <template #default="scope">
          <el-form-item
            label=""
            label-width="0"
            :prop="[ruleKey, scope.$index, column.prop]"
            :rules="column.configEdit?.rules"
          ...
          >
            ...
            <!-- 单个单元格编辑 -->
            <template v-if="column?.hasOwnProperty('configEdit')">
              <single-edit-cell
              ...
              >
                <slot
                  ...
                  :name="column.configEdit.editSlotName"
                  :scope="scope"
                />
              </single-edit-cell>
            </template>
            <div v-if="!column?.hasOwnProperty('configEdit')">
